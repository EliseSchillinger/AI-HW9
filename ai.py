...
from chess import chess_game_logic
...
class AI(BaseAI):
    ...
    def game_updated(self) -> None:
        ...
        """
        # GET FEN STRING HERE <--- from other already made data structure (game?)
        #UPDATE OWN INTERNAL OBJECTS
    ...
    def make_move(self) -> str:
        ...
        # ID-Depth Limited Minimax
        depth = 1
        past_moves = self.game.history
        last_8_moves = []
        fen_string = self.game.fen

        if len(past_moves) > 7: # read the last 8 moves
            for n in range(1,9):
                last_8_moves.append(past_moves[-n])
        move = chess_game_logic.max_choice(fen_string, 0, last_8_moves) # action is the move, takes the state (fen) and current depth
        while(True):
            move = chess_game_logic.max_choice(fen_string, depth, last_8_moves)
            depth += 1
            print(move)
            return move
